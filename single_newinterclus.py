...
#        mat = [[0.0]*len(lookup)]*len(sentset)
	mat = range(0,len(sentset));
	print len(sentset),len(lookup)
        for ps,ind in sentset:
		vec = [0.0]*len(lookup)
                slen = len(ps)
		print mat[ind] is mat[ind-1]
		f = open(str(ind)+".txt",'w')
                for t in ps:
			if (t!= "thisisempty"):
                        	vec[lookup.index(t)] += 1
#				print >>f, t,lookup.index(t)
		mat[ind] = vec;
		print >>f,"\n", mat[ind],"\n", lookup
		f.close()
...
try:
	worddict = pickle.load(open("worddicst.dict","rb"))
except:
	worddict = [];
...
#	sentlist = [t for t in sentlist if t not in stopwords]
	...
	ind += 1;
#stop the epsilon rounding. it is sort of stupid.
...
batch_dist = scipy.spatial.distance.squareform(scipy.spatial.distance.pdist(tf,'cosine'))

sort_batch = batch_dist.reshape(-1).argsort()
columns,dontcare = batch_dist.shape

#round stupid epsilons away
batch_dist = np.around(batch_dist, decimals=8)
